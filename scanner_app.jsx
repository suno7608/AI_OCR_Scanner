...
const ADMIN_PASSWORD = "admin1234"; // 배포 시 변경 권장
...
export default function App() {
  ...
  const saveUsers = async (list) => {
    setUsers(list);
    try {
      await window.storage.set("users", JSON.stringify(list), true);
    } catch (e) {
      console.error(e);
    }
  };
    ...
    return (
      <CreateUser
        ...
        onCreate={async (name, pin) => {
          await saveUsers([...users, { name, pin }]);
          setCurrentUser(name);
          setStage("home");
        }}
      />
  ...
  if (stage === "setPinExisting") {
    ...
    return (
      <SetPin
        ...
        onSet={async (pin) => {
          await saveUsers(users.map((u) => u.name === pendingUser ? { ...u, pin } : u));
  ...
  if (stage === "enterPin") {
    const u = users.find((x) => x.name === pendingUser);
    return (
      <EnterPin
        name={pendingUser}
        correctPin={u?.pin}
  ...
  if (stage === "admin") {
    return (
      <AdminPanel
        users={users}
        onBack={() => setStage("pickUser")}
        onDeleteUser={async (name) => {
          await saveUsers(users.filter((u) => u.name !== name));
          // 해당 사용자 데이터도 정리
          try {
            await window.storage.delete(`cards:${name}`);
            await window.storage.delete(`receipts:${name}`);
          } catch (e) { console.error(e); }
        }}
        onResetPin={async (name, newPin) => {
          await saveUsers(users.map((u) => u.name === name ? { ...u, pin: newPin } : u));
        }}
      />
    );
  }
            ...
            <div style={{ display: "flex", flexWrap: "wrap", gap: 10 }}>
              {users.map((u) => (
                <button key={u.name} onClick={() => onPick(u.name)}
                  ...
                  {!u.pin && <span style={{ fontSize: 11 }}>🔓</span>}
                  {u.pin && <span style={{ fontSize: 11 }}>🔒</span>}
                  {u.name}
                </button>
              ))}
...
// ── PIN 입력 (잠금 해제) ──
function EnterPin({ name, correctPin, onSuccess, onCancel }) {
  const [pin, setPin] = useState("");
  const [err, setErr] = useState("");
  return (
    <AuthShell title={name} subtitle="PIN을 입력하세요." onCancel={onCancel}>
      <PinPad value={pin} onChange={(v) => {
        setPin(v);
        if (v.length === 4) {
          if (v === correctPin) onSuccess();
          else { setErr("PIN이 틀렸습니다. 잊으셨다면 관리자에게 문의하세요."); setPin(""); }
        }
      }} />
      {err && <p style={{ textAlign: "center", color: C.accent, fontSize: 13, marginTop: 20 }}>{err}</p>}
    </AuthShell>
...
function AdminLogin({ onSuccess, onCancel }) {
  ...
  const [err, setErr] = useState("");
  const submit = () => { if (pw === ADMIN_PASSWORD) onSuccess(); else setErr("비밀번호가 틀렸습니다."); };
  return (
...
function AdminPanel({ users, onBack, onDeleteUser, onResetPin }) {
  ...
  if (resetting) {
    return (
      <AuthShell title="PIN 재설정" subtitle={`${resetting} 님의 새 PIN을 설정합니다.`} onCancel={() => setResetting(null)}>
        <AdminPinReset name={resetting} onDone={async (pin) => { await onResetPin(resetting, pin); setResetting(null); }} />
      </AuthShell>
    );
  }
          ...
          {users.map((u) => (
            <div key={u.name} style={{ background: C.card, border: `1.5px solid ${C.line}`, borderRadius: 14, padding: 16 }}>
              ...
              <div style={{ display: "flex", gap: 8, marginTop: 12 }}>
                ...
                {confirmDel === u.name ? (
                  <button onClick={() => { onDeleteUser(u.name); setConfirmDel(null); }}
...
function Home({ user, onSwitchUser }) {
  ...
  const cardKey = `cards:${user}`;
  const rcptKey = `receipts:${user}`;

  useEffect(() => {
    (async () => {
      setLoaded(false);
      try {
        const c = await window.storage.get(cardKey);
        setCards(c ? JSON.parse(c.value) : []);
      } catch { setCards([]); }
      try {
        const r = await window.storage.get(rcptKey);
        setReceipts(r ? JSON.parse(r.value) : []);
      } catch { setReceipts([]); }
      setLoaded(true);
    })();
  }, [user]);
  ...
  const persistCards = async (list) => {
    setCards(list);
    try { await window.storage.set(cardKey, JSON.stringify(list)); } catch (e) { console.error(e); }
  };
  const persistReceipts = async (list) => {
    setReceipts(list);
    try { await window.storage.set(rcptKey, JSON.stringify(list)); } catch (e) { console.error(e); }
  };
  ...
  const handleFiles = async (fileList) => {
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      try {
        ...
        const result = await parseWithClaude(base64, mediaType);
...
// ───────────────────────── Claude 파싱 ─────────────────────────
async function parseWithClaude(base64, mediaType) {
